...
const movieDbClient = async (url, method = 'GET', body) => {
  ...
  if (Array.isArray(movie.results)) {
    for (let index = 0; index < movie.results.length; index++) {
      movie.results[index].images = {};

      if (movie.results[index].poster_path) {
        movie.results[index].images.poster = getFullImageURLs(movie.results[index].poster_path);
      }
      if (movie.results[index].backdrop_path) {
        movie.results[index].images.backdrop = getFullImageURLs(movie.results[index].backdrop_path);
      }
    }
  } else {
    movie.images = {};
