 ...
 * @param {string|number} query additional query
...
const movieDbClient = async (url, query, method = 'GET', body) => {
  ...
  if (Array.isArray(data.results)) {
    data.results = data.results.map((movie) => {
      ...
      const movieToCamelCase = Object.keys(movie).map((key) => [toCamelCase(key), movie[key]]);
      movie = Object.fromEntries(movieToCamelCase);
      return movie;
    });
  } else {
    data.images = {};
    ...
    data = Object.fromEntries(dataToCamelCase);
  }
  // snake_case to camelCase below
...
/**
 * Get a list of movies belonging to given category
 *
 * @export
 * @param {string|number} id Id of the category to get a list of movies belonging to given category
 * @return {Promise<Object|null>}  movie object
 */
