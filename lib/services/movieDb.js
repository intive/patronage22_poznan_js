 ...
 */
const movieDbClient = async (url, category, method = 'GET', body) => {
  const token = process.env.MOVIEDB_API_KEY;
  const baseUrl = 'https://api.themoviedb.org/3';
  const wholeUrl = `${baseUrl}/${url}?api_key=${token}&language=en-US${category}`;
...
export async function getListOfMoviesByCategoryId(id) {
  ...
  const category = `&sort_by=popularity.desc&with_genres=${id}`;
  return movieDbClient(url, category);
}
